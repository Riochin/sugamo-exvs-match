...
describe('PlayerCard', () => {
  it('プレイヤー名を表示する', async () => {
    const wrapper = await mountCard({ id: 'p1', name: 'Alice', team: 'FIRST', title: null, mainUnit: null, iconUrl: null })
  ...
  it('team === SECOND のとき「2軍」バッジを表示する', async () => {
    const wrapper = await mountCard({ id: 'p2', name: 'Bob', team: 'SECOND', title: null, mainUnit: null, iconUrl: null })
  ...
  it('title が設定されているとき称号を表示する', async () => {
    const wrapper = await mountCard({ id: 'p1', name: 'Alice', team: 'FIRST', title: '勇者', mainUnit: null, iconUrl: null })
  ...
  it('mainUnit が設定されているとき機体名を表示する', async () => {
    const wrapper = await mountCard({ id: 'p1', name: 'Alice', team: 'FIRST', title: null, mainUnit: 'ユニコーンガンダム', iconUrl: null })
    expect(wrapper.text()).toContain('ユニコーンガンダム')
  })

  it('iconUrl が設定されているときアバター画像を表示する', async () => {
    const wrapper = await mountCard({ id: 'p1', name: 'Alice', team: 'FIRST', title: null, mainUnit: null, iconUrl: '/icons/alice.png' })
    const img = wrapper.find('img')
    expect(img.attributes('src')).toBe('/icons/alice.png')
  })

  it('iconUrl が null のときデフォルト画像を表示する', async () => {
    const wrapper = await mountCard({ id: 'p1', name: 'Alice', team: 'FIRST', title: null, mainUnit: null, iconUrl: null })
    const img = wrapper.find('img')
    expect(img.attributes('src')).toBe('/icons/default.png')
  })
})
